# M08 · Guía del módulo: CI/CD + Debugging

> 🎁 **Proyecto de referencia.** En el repo del curso, este módulo incluye una carpeta `proyecto/`: un proyecto Playwright **autocontenido y ejecutable** con el **framework completo** (todos los módulos + CI) ya armado, independiente del monorepo. Úsalo como **solución de referencia**: ábrelo aparte y corre `pnpm install` → `cp .env.example .env` → `pnpm test`. Los pasos de esta guía siguen construyendo **tu** proyecto incremental; `proyecto/` es el "ya resuelto".
...
Aparece **`.github/workflows/`** — el cuarto del robot. Con eso el framework queda completo: el mismo suite que corres con `pnpm m7` ahora corre **en paralelo en GitHub Actions** cada vez que abres un PR.
...
Si no tienes remoto, vuelve a M06 (Git break: conectar el repo a GitHub).
...
| `git` con remoto | `git remote -v` | M06 Git break — conectar repo a GitHub |
...
Añade los scripts de M08 al `package.json`:
