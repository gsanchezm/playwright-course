...
- `pages/ProfilePage.ts` + `pages/PizzaCustomizerModal.ts` + `pages/CheckoutPage.ts` ampliado (radio de pago, `<select>` de tarjeta, tooltips, confirmación de orden en 2 pasos) — interacciones con [widgets nuevos](/docs/playwright/m5-interacciones), 8 escenarios más repartidos en `tests/widgets/` (uno por funcionalidad).
