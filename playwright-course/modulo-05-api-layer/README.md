...
```bash
pnpm test:api                                    # sólo tests/api/ + modulo-05
pnpm exec playwright test --project=api          # equivalente
pnpm exec playwright test tests/api/auth.spec.ts # un spec específico
pnpm exec playwright test --grep @api            # por tag
