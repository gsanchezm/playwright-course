...
| Tu fixture custom `loginPage` (M03+M04) | Page Object inyectado ya construido |
| Tu fixture custom `authenticatedPage` (M04) | Página YA logueada antes de empezar el test |
...
**Son ~6 líneas, idénticas, en cada test.** Ahora imagina que en M04 vas a tener esto:
...
Solo para que veas la sintaxis a la que estás corriendo. **No lo implementes en M01** — es spoiler de M04:

```ts
// tests/fixtures/auth.fixture.ts (esto NACE en M04, no aquí)
...
Anatomía sin entrar a fondo (esto se profundiza en M04):
...
- En este curso, los siguientes módulos te llevan paso a paso:
  - **M03** → crearás Page Objects (todavía sin envolverlos en fixtures).
  - **M04** → envuelves esos Page Objects en fixtures custom + storage state reutilizable.
  - **M05** → fixture `apiClient` para tests de API sin pasar por la UI.
