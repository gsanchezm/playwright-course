...
**Pieza que suma al framework:** `tests/ejemplo.spec.ts` plano, sin POM, sin fixtures, sin data-driven. **Todo el dolor es deliberado.**
...
| `tests/setup/`, `fixtures/`, `helpers/`, `.auth/` | M05-M06 | Setup project + custom fixtures |
...
>
> 🔍 **Ojo con el `playwright.yml` y el `tests/`:** el installer ya deja un workflow de GitHub Actions y una carpeta `tests/` con un `example.spec.ts` de demo. El workflow queda **latente** (lo activamos de verdad en M08). La carpeta `tests/` **sí la usamos** — ahí viven tus specs — pero borras el `example.spec.ts` de demo (correría bajo nuestro `testMatch`). En el Paso 4 verás por qué.
...
### Paso 0 — Pre-requisitos y entrar al proyecto de M01

**0.1 — Confirma prerequisitos y sitúate en el `proyecto/` de este módulo**
- **Qué hago:** reviso las versiones y entro al `proyecto/` de M01 — es **aquí**, dentro de la carpeta del módulo, donde vas a construir todo con el installer. A diferencia de M02-M06 (donde el `package.json` ya viene y arrancas con `pnpm install`), en M01 el `package.json` lo **genera el installer** en el Paso 1.
  ```bash
  node -v                # v24.x.x
  pnpm -v                # 10.x.x o superior
  git --version          # Git instalado
  cd modulo-01-smoke-feo/proyecto
  git log --oneline -1   # ves el primer commit que hiciste en M00 (el repo Git ya existe)
  ```
- **Por qué:** el repo Git lo creaste en M00 (`git init` + primer commit); **no** hay un proyecto Playwright compartido en la raíz del curso. Cada módulo trabaja dentro de su propio `proyecto/`, y el de M01 es el que llenas con el installer. El `proyecto/` que ves ya resuelto es la **referencia** para comparar (córrela aparte con `pnpm install` → `cp .env.example .env` → `pnpm test`); los pasos de abajo son cómo se armó.
- **Cómo verifico:** `pwd` termina en `modulo-01-smoke-feo/proyecto` y `git log --oneline -1` muestra tu commit de M00.

> 💡 **Para el facilitador:** si alguien no tiene Node/pnpm, mándalo al módulo [`00-setup/`](../../00-setup/) del monorepo antes de continuar. No hagas troubleshooting de setup aquí. Recalca el modelo: **no hay proyecto raíz compartido** — cada módulo vive en su propio `proyecto/`, y M01 es donde ese `proyecto/` nace con el installer.
...
- **Qué hago:** desde el `proyecto/` de este módulo (donde te dejó el Paso 0), corro:
  ```bash
  ...
  | **Where to put your tests?** | `tests` (default) | Lo aceptas; en el Paso 4 lo formalizamos con un `testMatch` explícito |
...
- **Qué hago:** inspecciono el scaffold tal cual quedó:
  ...
  ```
- **Por qué:** antes de reconciliar hay que **ver el punto de partida**. Tres cosas que vas a tocar en los Pasos 3-4: (1) el `.gitignore` del installer **no** ignora `.env`; (2) el config trae un bloque `dotenv` **comentado** — el installer ya te dejó el hook, solo hay que encenderlo; (3) `testDir` apunta a `./tests`; en el Paso 4 lo formalizas (`testDir: "."` + `testMatch` explícito a `tests/`), sin sacar los specs de `tests/`.
...
> **Archivo que se crea en este paso:** `.env` (en el `proyecto/` de este módulo, **gitignored**). El `.env.example` ya existe versionado.

**2.1 — Copiar la plantilla**
- **Qué hago:** desde el `proyecto/` de este módulo, `cp .env.example .env`
...
> ⚠️ **Importante para el facilitador:** abre `.gitignore` con los alumnos y muéstrales la línea `.env`. Repite en voz alta: **"el `.env` real NUNCA se commitea, solo el `.env.example`"**. Este es el momento de hablar de secrets, leaks de tokens en GitHub y por qué CI usa `secrets.*` (lo veremos en M08).
...
> **Archivo que se edita en este paso:** `.gitignore` (en la raíz de `proyecto/`). **No lo creas desde cero**: el installer ya generó uno (con `/playwright/.cache/`, `/playwright/.auth/`, `/test-results/`, `/playwright-report/`, `/blob-report/`). El problema: **NO ignora `.env`**. Lo arreglas **antes** de crear `.env`, para que tus credenciales nunca entren al staging por accidente.
...
- **Qué hago:**
  ...
  cat .gitignore
  ```
- **Por qué:** el `.gitignore` del installer ya cubre los reportes y el `node_modules/`, y trae `/playwright/.auth/` (lo usaremos en M06). Lo que **falta** son las dos líneas críticas de secretos: `.env` y `.auth/`. Reconocer qué ya está te evita duplicar líneas.
...
- **Qué hago:** **añado** (no reemplazo) al final del `.gitignore` que ya existe. Abro el archivo en VS Code (`code .gitignore`) y pego al final estas líneas:
  ...
  .auth/
  ```
- **Por qué:** las entradas `.env` y `.auth/` son **críticas**. Si las olvidas, terminas pusheando secrets al repo. En M00 ya viste el **concepto** de `.gitignore` con un mínimo; aquí lo **consolidas** mergeando el del installer con esas dos líneas. (`.auth/` aún no existe, pero lo dejas listo: en M06 guardará el `storageState` de sesión.)
- **Cómo verifico:**
  ```bash
...
>
> **El estado M01 contiene lo mínimo:** `import "dotenv/config"` (descomentado), `baseURL` desde `process.env`, timeouts generosos (cold start de Render) y **un solo project** `ui-anon`. Todavía NO hay: setup project, `storageState` (M06), multi-browser (M08), project `api` (M07), ni la matrix de CI real (M08).
...
> **Archivo que se edita en este paso:** `playwright.config.ts` (en `proyecto/`) — el que generó el installer.
...
- **Qué hago:** comparo lo que trae el installer contra lo que necesita M01. Esta es la tabla de reconciliación (la lección del módulo):
  ...
  |---|---|---|
  | `testDir: "./tests"` | `testDir: "."` + `testMatch: [/tests\/.*\.spec\.ts/]` | Deja el patrón de descubrimiento explícito; los specs viven en `tests/` |
  | `projects: [chromium, firefox, webkit]` | **solo** `ui-anon` (anónimo) | Multi-browser distrae en M01; **firefox/webkit nacen en M08 (CI) y el `ui-chromium` autenticado en M06** |
...
- **Qué hago:** borro **solo** el archivo `tests/example.spec.ts` desde el **explorador de VS Code** (click derecho sobre el archivo → *Delete*). Si prefieres la terminal, no hay comando neutral: bash `rm tests/example.spec.ts` · 🪟 PowerShell `Remove-Item tests/example.spec.ts`.
- **Por qué:** el installer dejó en `tests/` un `example.spec.ts` de demo (apunta a `playwright.dev`, no a OmniPizza). Como tus specs también viven en `tests/`, ese demo **sí entra** en el `testMatch` y correría — lo borras para no arrastrar un test de ejemplo. **La carpeta `tests/` la conservas**: ahí van tus specs. (El workflow `.github/workflows/playwright.yml` lo **conservas** también: queda latente hasta M08.)
- **Cómo verifico:** `tests/example.spec.ts` ya no existe; la carpeta `tests/` sigue ahí (es donde pondrás tus specs).
...
| `testMatch: [/tests\/.*\.spec\.ts/]` | Regex que filtra, dentro de `testDir`, qué archivos cuentan como tests | Solo `*.spec.ts` dentro de `tests/`; el demo `example.spec.ts` del installer también entraría — por eso lo borraste en 4.3, para no arrastrar un test de ejemplo |
...
| `projects: [...]` | Cada project es una configuración de ejecución con nombre (navegador, viewport, overrides de `use`); la suite corre una vez por project | En M01 uno solo para no distraer; en M08 vuelven firefox/webkit y en M06 se monta el setup project |
| `name: "ui-anon"` | Identificador del project: lo que pasas en `--project=ui-anon` y lo que ves en el report | `anon` porque M01-M05 corren **sin sesión** (su lección ES el login); el `ui-chromium` autenticado nace en M06. El prefijo `ui-` (vs `api` de M07) es la convención del curso |
...
> **Archivo que se crea en este paso:** `tsconfig.json` (en `proyecto/`). **Este sí lo creas desde cero**: el installer de Playwright **no** genera un `tsconfig.json` (asume el default de TS). Lo añadimos nosotros para fijar `strict`, los tipos de Node y el `include` del curso.
...
- **Qué hago:** TypeScript necesita saber cómo compilar tus specs; creo el archivo en `proyecto/` con el contenido de abajo. La vía más simple es el **editor**: crea `tsconfig.json` en VS Code (`code tsconfig.json`) y pega el JSON. O desde la terminal:
...
- **Qué hago:** reviso los scripts que dejó el installer (suele dejar `scripts` casi vacío) y **añado** los que falten:
  ...
  cat package.json
  ```
- **Por qué:** `pnpm m1` es **azúcar sintáctica** — equivale a `pnpm exec playwright test --project=ui-anon`. Un atajo memorable evita teclear el comando largo en cada corrida.
...
"scripts": {
  ...
  "m1": "playwright test --project=ui-anon"
}
```

> 💡 **Para el facilitador:** explica que `pnpm m1` es **azúcar sintáctica** — equivale a `pnpm exec playwright test --project=ui-anon`. Los alumnos pueden inspeccionar cualquier script con `cat package.json`.
...
> **Archivo que se crea en este paso:** `tests/ejemplo.spec.ts`.

**7.1 — Crear el archivo del spec**
- **Qué hago:**
  ```bash
  # 1. La carpeta tests/ ya existe (la dejó el installer; borraste su example.spec.ts en el Paso 4)

  # 2. Crea y abre el archivo del spec en VS Code (se crea al guardar)
  code tests/ejemplo.spec.ts
  ```
- **Por qué:** el `testMatch` del config (`/tests\/.*\.spec\.ts/`) solo recoge archivos `*.spec.ts` dentro de `tests/`. La ubicación y el nombre importan para que Playwright lo descubra.
- **Cómo verifico:** tras guardar, `ls tests/` muestra `ejemplo.spec.ts`.
...
- **Qué hago** (tres formas, en orden de utilidad pedagógica):
  ...
  pnpm exec playwright test --headed --project=ui-anon
...
- **Qué hago:** corro el smoke en modo depuración:
  ```bash
  pnpm exec playwright test --project=ui-anon --debug
...
- **Por qué:** el config genera el reporter `html` (lo dejaste en el Paso 4). El HTML report es el **artefacto compartible** del resultado: en CI (M08) lo subiremos como artifact para que el equipo lo revise sin re-correr nada. El **Trace Viewer** (dentro del report, al fallar) es la "caja negra" del test: timeline, DOM snapshot por acción, network y consola.
...
**Pregunta al grupo:** *"si añadieran un tercer smoke, ¿cuántas líneas duplicarían?"* — la respuesta esperada es **~8**. Esa es la deuda que M03 y M04 van a pagar.
...
- **Qué hago:** abro `reto.spec.ts` y escribo TC-003 **copiando deliberadamente** las mismas líneas que ya viste duplicadas. Cada TODO trae **qué hacer**, **pista** (el método de Playwright) y **cómo verificar**. No intento ser elegante — el dolor es el ejercicio. Al terminar:
  ...
  ```
- **Por qué:** este tercer smoke vuelve a duplicar ~8 líneas de login. Medir cuántas copiaste pone número a la deuda técnica que M04 va a saldar con el Page Object Model.
- **Cómo verifico:** el test pasa en verde **y** al final del archivo puedes responder las 3 preguntas del comentario final.

> 💡 **Para el facilitador:** al cerrar el reto, abre `ejemplo.spec.ts` y `reto.spec.ts` lado a lado y pide al grupo contar las líneas idénticas entre TC-001 / TC-002 / TC-003. Anota el número en el pizarrón — lo vamos a destruir en M04.
...
- **Qué hago:** desde el `proyecto/` de este módulo, agrego al staging (con rutas relativas a `proyecto/`) lo que generó el installer y lo que creaste tú en este módulo:
  ```bash
...
> Ejecuta desde el proyecto de referencia autocontenido: **`cd proyecto`** (su `package.json` define estos scripts). Antes de la 1ª corrida: `pnpm install` → `pnpm install:browsers` → `cp .env.example .env`.
...
En el próximo módulo vas a dominar los **locators**: la **jerarquía** de Playwright (`getByRole` → `getByLabel`/`getByText` → `getByTestId`), cómo **filtrar y acotar** el elemento exacto (`.filter()`, `.nth()`, `.first()`, `.and()`/`.or()`, scoping dentro de un contenedor) sin acoplarte a un DOM frágil, y cómo generarlos rápido con **Codegen**. Es el fin de los selectores inline que copiaste en este smoke — y la base para la parametrización data-driven que llega en M03.
