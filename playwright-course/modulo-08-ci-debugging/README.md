# Módulo 08 — CI/CD + Debugging con Trace Viewer
...
> 🎁 **Proyecto de referencia — [`proyecto/`](proyecto/).** Este módulo trae una carpeta `proyecto/`: un proyecto Playwright **autocontenido y ejecutable** con el **framework completo** (los 8 módulos + CI) ya armado (su propio `package.json` · `playwright.config.ts` · `tsconfig.json` · `.env.example`, independiente del monorepo del curso). Es la **solución de referencia** para comparar: ábrela aparte y corre `pnpm install` → `cp .env.example .env` → `pnpm test`. Los pasos de este README siguen construyendo **tu** proyecto incremental; `proyecto/` es el "ya resuelto". Detalles en [`proyecto/README.md`](proyecto/README.md).

## 🏗️ Arquitectura al terminar este módulo (estado FINAL del framework)

Aparece **`.github/workflows/`** — el cuarto del robot. Con eso el framework queda completo: el mismo suite que corres con `pnpm test` ahora corre **en paralelo en GitHub Actions** cada vez que abres un PR.
...
│   └── setup/                      ← (M06) auth.setup.ts
├── types/                          ← (M03) omnipizza.d.ts
├── modulo-00-git-esencial/         ← (lecciones — no contribuyen al framework)
├── modulo-01-smoke-feo/            ← (M01)
├── modulo-02-locators/             ← (M02)
├── modulo-03-data-driven/          ← (M03)
├── modulo-04-pom/                  ← (M04)
├── modulo-05-fixtures/             ← (M05)
├── modulo-06-setup/                ← (M06)
├── modulo-07-api-layer/            ← (M07)
├── modulo-08-ci-debugging/         ← 🆕 ESTE MÓDULO
│   ├── README.md
...
> 🔀 **La matriz cross-browser vive aquí.** En la estructura de 10 módulos, los projects `ui-firefox` y `ui-webkit` se consolidan **en este módulo de CI** — que es donde correr la suite en varios navegadores en paralelo cobra sentido conceptual. Antes se introducían junto al setup; ahora la matriz multi-navegador es una responsabilidad de CI.
...
Antes de tocar CI, asegúrate de que el framework de M07 corre verde **en local** y de que tienes las herramientas de GitHub listas.

**0.1 — Verifica que M07 está verde**
- **Qué hago:** desde `proyecto/` (de este módulo), corro la suite anterior y el typecheck.
  ```bash
  pnpm m7            # API layer corre en verde
  pnpm typecheck     # sin errores
  ```
- **Por qué:** CI sólo amplifica lo que ya tienes. Si M07 falla en tu laptop, va a fallar en 7 jobs paralelos a la vez — y vas a debuggear en el peor sitio (un runner remoto) en vez de en tu máquina.
...
- **Cómo verifico:** `gh auth status` dice *"Logged in"*; `git remote -v` lista un `origin` apuntando a `github.com`. Si falta `gh`: macOS → `brew install gh`; Windows → `winget install --id GitHub.cli`; Linux → [`cli.github.com`](https://cli.github.com/). Si falta el remoto, vuelve a M04 (Paso 0.3: crear el repo en GitHub y conectarlo).

> 💡 **Para el facilitador:** este módulo asume que el repo ya tiene `origin` en GitHub. Si alguien todavía no lo tiene, dedícale 5 minutos al setup del remoto (HTTPS o SSH; ver M04 Paso 0.3) antes de continuar — sin remoto no hay CI.

---

### Paso 1 — Dependencias requeridas

**M08 no añade paquetes npm.** Lo que se agrega es **YAML** (workflows) y un reporter adicional (`junit`) que ya viene en `@playwright/test`.
...
- **Qué hago:** listo los 4 paquetes que ya instalaste en M01.
  ...
  ```
- **Por qué:** M08 no instala nada nuevo de npm; sólo necesita que la base de M01 siga ahí. El reporter `junit` y el `github` no son paquetes: vienen dentro de `@playwright/test`.
...
- **Qué hago:** reviso que tengo `gh`, un remoto y permisos de escritura sobre el repo.
  ...
  | `git` con remoto | `git remote -v` | M04 Paso 0.3 — conectar repo a GitHub |
...
| `env:` a nivel job | Variables disponibles en **todos** los steps. Las credenciales vienen de `secrets.*` (encriptadas — Paso 5); `DEFAULT_COUNTRY: MX` va en texto plano porque **no es un secreto**: es el mercado por defecto que consumen los tests data-driven (M03) |
...
> **📐 Config — cambios vs M07 (estado FINAL del curso)**
> ```diff
...
>     },
>     projects: [
>       // sin cambios: ui-anon (M01-M04), setup, ui-chromium/firefox/webkit, api
>     ]
>   })
> ```
> **Se mantiene:** TODOS los projects (`ui-anon`, `setup`, los 3 browsers, `api`) — **M08 ya no añade projects**. **Entra:** flags de CI (`fullyParallel`, `forbidOnly`, `retries`, `workers`), reporters condicionales (`github` + `junit` sólo en CI) y `video: "retain-on-failure"`.

M08 añade los **flags de comportamiento en CI**: `fullyParallel`, `retries`, `workers`, reporters extra (`github`, `junit`), `forbidOnly` y `video` al fallar. Editas **un solo archivo** (`playwright.config.ts`) en dos micro-pasos. (El project anónimo `ui-anon` para flujos negativos **ya existe desde M01** — corre M01-M04 y cualquier `*.anon.spec.ts`.)
...
- **Cómo verifico:** `pnpm exec tsc --noEmit` sigue verde; en local `pnpm m8` corre con 0 retries (no ves reintentos en la salida).

> 🔍 **Detalle que parece obvio — `process.env.CI`**
> **Qué pasa si lo cambias:** si la seteas a mano en local, tu máquina se comporta como CI (2 retries, `forbidOnly`, reporters extra) y pierdes justo las ventajas de correr en local. Para *simular* CI a propósito, sí la seteas temporalmente: `CI=true pnpm m8` (bash) / `$env:CI="true"; pnpm m8` (PowerShell).
...
- **Cómo verifico:** `pnpm exec tsc --noEmit` verde; al simular CI (`CI=true pnpm m8` en bash · 🪟 `$env:CI="true"; pnpm m8` en PowerShell) ves anotaciones estilo GitHub en la salida.

> 🔍 **Detalle que parece obvio — `video: "retain-on-failure"` / `screenshot: "only-on-failure"`**
> **Qué es:** video y screenshot se guardan **únicamente cuando el test falla**, mismo principio que el `trace`. En M08 sólo el `video` es nuevo: el `screenshot: "only-on-failure"` ya lo traías de módulos anteriores.
...
```ts
// playwright.config.ts — Estado en M08 (FINAL)
...
export default defineConfig({
  ...
  projects: [
    ...
    {
      // ui-anon: existe desde M01 (M01-M04 anónimos) + flujos negativos *.anon.spec.ts
      name: "ui-anon",
...
**3.3 — Añadir los scripts de M08 al `package.json`**
- **Qué hago:** agrego tres scripts al bloque `"scripts"` de `package.json`.
  ```json
  "scripts": {
    "m8": "playwright test modulo-08-ci-debugging --project=ui-chromium",
    "test:smoke": "playwright test --grep @smoke",
    "test:regression": "playwright test --grep @regression"
  }
  ```
- **Por qué:** `pnpm m8` es el atajo del módulo; `test:smoke` / `test:regression` filtran por las etiquetas `@smoke` / `@regression` que `ejemplo.spec.ts` ya usa en sus `describe`. Las mismas etiquetas las consume el reto (canary diario con `--grep @smoke`).
- **Cómo verifico:** `pnpm test:smoke` corre **solo** el `describe("M08 — smoke canary @smoke")`; `pnpm m8` corre el módulo en `ui-chromium`.
...
- **Cómo verifico:** `git branch --show-current` imprime `feature/m08-ci`.

**8.2 — Versionar tu trabajo (commit)**
- **Qué hago:** agrego los archivos del módulo y los de infraestructura, y commiteo.
  ```bash
  git add .github/workflows/playwright.yml playwright.config.ts package.json modulo-08-ci-debugging
  git commit -m "feat(m08): habilita CI workflow + flags de CI en config"
  ```
...
- **Cómo verifico:** `git log --oneline -1` muestra tu commit `feat(m08): ...`.
...
- **Cómo verifico:** `git status` dice *"Your branch is up to date with 'origin/feature/m08-ci'"*.
...
> Desde la carpeta [`proyecto/`](proyecto/) de este módulo: `cd proyecto` · `pnpm install` · `cp .env.example .env`. Los comandos de abajo corren dentro de ese `proyecto/`.

- **Comando del módulo:** `pnpm m8`
...
**🪟 Windows / PowerShell:** las variables de entorno se setean con `$env:VAR="x"; pnpm m8` — **no** con `VAR=x pnpm m8` (esa sintaxis es de bash y en PowerShell falla). Ejemplos:

```powershell
# Forzar el fallo del demo en PowerShell
$env:DEMO_FAIL="1"; pnpm exec playwright test modulo-08-ci-debugging --project=ui-chromium
