...
- `pages/ProfilePage.ts` + `pages/PizzaCustomizerModal.ts` + `pages/CheckoutPage.ts` ampliado (radio de pago, `<select>` de tarjeta, tooltips, confirmación de orden en 2 pasos) y `tests/interacciones-nuevas.spec.ts` — interacciones con widgets nuevos (8 tests).
